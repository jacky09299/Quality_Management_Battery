...
Incorporates SOH_0 (Max capacity of first 5 cycles) as an explicit feature.
"""
import pandas as pd
import numpy as np
import os
...
def build_datasets(bat_feats_full, bat_feats_short, bat_sohs, bat_rows):
    ...
    for bat in bat_feats_full:
        ...
        # Calculate robust SOH_0 from the first 5 cycles
        soh_0 = float(np.max(sohs_list[:min(5, len(sohs_list))]))
        ...
        for k in range(n):
            ...
            # Appended soh_0 at the end of the feature array
            xf = np.concatenate([cur_f, ff_first, rel_f, rate_f_norm, rate2_f_norm, [k, k/max(n-1,1), amb, soh_0]])
            ...
            # Appended soh_0 at the end of the short feature array
            xs = np.concatenate([cur_s, fs_first, rel_s, rate_s, [k, k/max(n-1,1), amb, soh_0]])
...
def main():
    ...
    print("Building datasets with SOH_0 feature...")
